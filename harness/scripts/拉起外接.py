...
try:
    ...
    HEALTH = (
        _h.gsh_harness / "mcp-health.json"
        if _h.gsh_harness.is_dir()
        else CURSOR / "harness" / "mcp-health.json"
    )
    HOST_PATHS = Path(
        os.environ.get("HARNESS_HOST_PATHS")
        or (
            _h.gsh_harness / "host-paths.json"
            if (_h.gsh_harness / "host-paths.json").is_file()
            else CURSOR / "harness" / "host-paths.json"
        )
    )
except Exception:
