...
第三层是做法库：106 条技能、35 条职种路径、MCP 用途桩，覆盖完整制作流水线。技能从真实工作室流程蒸馏而来：检查表、表写入配方、灰盒与节拍验收、契约与提测口径被压成带输入/输出与不合格回退的 `SKILL.md`。职种文件只排步骤顺序；做法正文在技能里。安装后生成 `catalog.json` 给导演点名。点名一条只打开当前步；未点名的仍留在菜单里。
